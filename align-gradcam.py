...
if __name__ == '__main__':
    ...
    model_clip = load_clip_model(model_path='/home/ec2-user/CheXzero/checkpoints/pranav/best_64_0.0001_original_35000_0.864.pt', pretrained=False, context_length=77).float().cpu()
    model = model_clip
    model.eval()
 ...
 #   print(all_paths)
    for image_path in tqdm(all_paths,total=len(all_paths)):
        ...
        for image_caption in classnames: 
            ...
            p = '/home/ec2-user/CHEXLOCALIZE/CheXpert/align_out_val/'+ os.path.join(*image_path.split('/')[-3:]).replace('/','_').replace('.jpg','_'+str(image_caption)+'_map.pkl')
